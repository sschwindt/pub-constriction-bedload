...
coeffs = xlsread(sourceName, 2, 'D41:F43');
...
for i = 1:3
    ...
    interX1 = nanmin(cX);
    interXX = nanmax(cX);
    interX = interX1-0.03:(interXX-interX1)/100:interXX+0.01;
    xInterp(1:numel(interX),i)=interX;
    yInterp(1:numel(interX),i)=nan*(coeffs(i,1).*interX.^coeffs(i,2)+coeffs(i,3));
...
fPlotZetaHx(X,Xb,xInterp, Y,Yb,yInterp, 1)
disp('Data processed.');
